...
describe('Authorization URL', () => {
  ...
  it('should generate authorize URL with required parameters', () => {
    const url = client.getAuthorizeUrl(Configuration.GITHUB_CLIENT_ID, 'user:email', mockState, 'code');
  ...
  it('should include nonce when provided', () => {
    const url = client.getAuthorizeUrl(Configuration.GITHUB_CLIENT_ID, 'user:email', mockState, 'code', mockNonce);
    expect(url).toContain(`nonce=${mockNonce}`);
  });

  it('should include PKCE parameters when code challenge is provided', () => {
    const url = client.getAuthorizeUrl(Configuration.GITHUB_CLIENT_ID, 'user:email', mockState, 'code', mockNonce, mockCodeChallenge);
